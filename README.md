...
一个面向智能手机新品发布与影像系统分析的可复用智能体技能，用于从官方资料、创作者完整测评和第三方技术解读中提取证据，并生成结构化分析或高质量交互式 HTML 报告。
...
`examples/apple-2026/` 是这个技能 的第一个完整参考案例。
...
> 这个案例是**参考实现**，不是固定模板。实际分析时应根据机型、产品形态、已有证据和发布阶段动态增加、删除或调整章节。

## 影像分析维度

技能会根据设备实际能力选择相关维度，包括：
...
```text
使用这个技能 分析最新的小米 Ultra 旗舰，重点解读影像系统、完整测评和对其他 Android 旗舰的技术启示，并生成交互式 HTML 页面。
```
...
目前已经完成通用技能框架和 Apple 2026 参考案例。后续可以继续加入 Xiaomi Ultra、vivo X 系列、OPPO Find X 系列、Samsung Galaxy Ultra、Huawei Pura 等不同产品形态的案例，用来验证和扩展这套分析框架。
